...
public class InsuranceliangshanyizuService implements InsuranceLogin {
	...
	public TaskInsurance login(InsuranceRequestParameters parameter) {
		...
		try {
			...
			if ("true".equals(success)) {
				...
				String encrypted3 = encrypted("rl3v8wwr5g6vuemn", publicKeyExponent, publicKeyModulus);
				...
				// 获取图片验证码
				String loginurl21 = "http://118.122.8.171:8003/lsui/CaptchaImg";
				...
				WebRequest requestSettings = new WebRequest(new URL("http://118.122.8.171:8003/lsui/member/login.jspx"),
						HttpMethod.POST);
				String body = "" + "aac002=" + encrypted1 + "&aae005=" + "&username=" + encrypted1 + "&password="
						+ encrypted2 + "&netysku=" + encrypted3 + "&loginid_type=idcard" + "&phonecode=" + code1 + ""
						+ "&usertype=aac001" + "&checkCode=" + code1;
				...
				if (contentAsString.contains("该用户名不存在")) {
					System.out.println("登陆失败！该用户名不存在");
				...
				} else if (contentAsString.contains("密码不正确!") || contentAsString.contains("密码错误!")) {
					...
					try {
						if (contentAsString.contains("凉山州社会保障网上办事大厅")) {
	...
	public TaskInsurance getAllData(InsuranceRequestParameters parameter) {
		TaskInsurance taskInsurance = taskInsuranceRepository.findByTaskid(parameter.getTaskId());
		tracer.addTag("InsuranceliangshanyizuService.crawler:开始执行爬取", parameter.toString());
		// 爬取解析基本信息
	...
	public static String encrypted(String num, String param1, String param2) throws Exception {
		...
		String path = readResource("liangshan.js", Charsets.UTF_8);
		engine.eval(path);
