    ...
    var interval_send = setInterval(function (){
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data.phase == "WAIT_CODE_SECOND" && data.phase_status == "FAIL") {//二次短信验证码发送失败！
                    $('#myModal').modal('hide');
                    clearInterval(interval_send); //停止间隔一秒调用
                    //显示错误信息
                    $('#msg').modal('show');
                ...
                } else if (data.phase == "WAIT_CODE_SECOND" && data.phase_status == "SUCCESS") {// 二次短信验证码已发送，请注意查收
                    $('#myModal').modal('hide');
                    clearInterval(interval_send); //停止间隔一秒调用
                    return;
    ...
    var interval_ver = setInterval(function (){
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data.phase == "VALIDATE" && data.phase_status == "SUCCESS" ) {//二次短信验证码验证成功
                    unicom();
                    ...
                    $('#alertMsg').text(data.description);
                    return;
                } else if (data.phase == "VALIDATE" && data.phase_status == "FAIL") {//二次短信验证码验证失败!
                    clearInterval(interval_ver); //停止间隔一秒调用
                    $('#myModal').modal('hide');
                    //显示错误信息
                    $('#msg').modal('show');
...
function nextSubmit(){
    $.ajax({
        type: "POST",
        url:"/h5/carrier/unicom/verifiCodeTwo",
...
function diffProvinceHtml(){
    ...
    //显示温馨提示
    $("#prompt").append("<small class='form-text text-muted'>温馨提示：<br />*使用二次短信验证码验证。<br /> </small>");
    ...
    var bindBtn = function(){
        btn.click(function(){
            ...
            $.ajax({
                type: "POST",
                url:"/h5/carrier/unicom/sendCodeTwo",
                data:$('#authform').serialize(),// 你的formid
                dataType : "json",
                beforeSend: function () {
