...
$('#myTab').find('li').click( function() {
    if(!$(this).hasClass("active")&&$(this).index()==0){
        ...
        $('#phone').find('input').attr("disabled","disabled");
        $('#email,#card').find('input').removeAttr("disabled");
        verifyForm1()
    }
    if(!$(this).hasClass("active")&&$(this).index()==1){
        $('#loginType').val('CITIZEN_EMAIL');
        $('#email').find('input').attr("disabled","disabled");
        $('#phone,#card').find('input').removeAttr("disabled");
        verifyForm2()
    }
    if(!$(this).hasClass("active")&&$(this).index()==2){
        $('#loginType').val('INSURANCE_CARD');
        $('#card').find('input').attr("disabled","disabled");
        $('#phone,#email').find('input').removeAttr("disabled");
        verifyForm2()
    }
});
