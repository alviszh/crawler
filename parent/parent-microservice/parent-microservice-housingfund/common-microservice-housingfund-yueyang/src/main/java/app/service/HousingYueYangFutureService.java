...
public class HousingYueYangFutureService extends HousingBasicService implements ICrawlerLogin,ICrawler{
	public static final Logger log = LoggerFactory.getLogger(HousingYueYangFutureService.class);
	
	@Autowired
	private LoginAndGetService loginAndGetService;
	@Autowired
	private HousingYueYangService housingYueYangService;
...
//				//公积金账号
//			    htmlpage = loginAndGetService.loginByCO_BRANDED_CARD(webClient,  messageLoginForHousing.getNum().trim(), messageLoginForHousing.getPassword().trim());
...
//			tracer.addTag("e", "baotougangjijin");
		...
		if(htmlpage!=null){
			...
			if(htmlpage.getWebResponse().getContentAsString().contains("登入成功")){
...
//			tracer.addTag("e", "baotougangjijin");
	...
	public TaskHousing getAllData(MessageLoginForHousing messageLoginForHousing) {
		...
		try {
			Future<String> future = housingYueYangService.getResult(messageLoginForHousing, taskHousing,
					webClient);
		} catch (Exception e) {
