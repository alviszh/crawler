...
$("#myTab").delegate("li", "click", function () {
    ...
    if($('#qr_state').text()==0&&!$(this).hasClass("active")&&$(this).index()==0) {
            setTimeout(interval_QRcode, 1000);
    }
})
...
function nextQRcode(){
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if((data.phase == "AGENT"||data.phase == "LOGIN")&& data.phase_status == "ERROR"){
                clearInterval(interval_login);
                $('#msg').modal('show');
                $('#alertMsg').text(data.description);
                $('#msg').find("button").bind("click",function(){
            ...
            if(data.phase == "LOGIN" && data.phase_status == "WAIT_SCAN"){
               ...
               // clearInterval(interval_login);
                ...
                $('.qrcode-login-btn').css("display","block");  //.attr("href","javacript:void(0);");
                ...
                $('.qrcode-login-btn').bind("click",function(){
                    ...
                    var mywin=window.open(data.qrUrl+'&ReturnUrl=https://i.jd.com/user/info','_self');
                    mywin.close();
...
//var iTime;
function interval_QRcode() {
    var interval_QRcode = setInterval(function (){
        console.log("********** "+$("#taskid").val());
        var taskid = $("#taskid").val();
        var is_state=true;
        $.ajax({
            type: "POST",
            url:"/h5/jd/checkQRcode",
            data:{
                taskid: taskid
            },
            dataType : "json",
            ...
            success: function(data) {
                if (data.phase == "NEED_SWEEP_QR_CODE"||data.phase_status == 'WAIT_SCAN') {}
                        ...
                        crawler(); //调用数据采集接口
                        $('#is_qr').text('yes');
                }else if (data.phase == "LOGIN" && data.phase_status == "ERROR") {
                    clearInterval(interval_QRcode);
                    //弹出提示框
                    $('#msg').modal('show');
                    $('#alertMsg').text(data.description);
...
$('#jump_a').click( function() {
    quit();
    window.location.href ="/h5/alipay/item"+location.search +"&source=tbqr"
});
