...
import app.service.HousingFundYiWuService;


@RestController  
@Configuration
@RequestMapping("/housing/yiwu")
public class HousingFundYiWuController extends HousingBasicController{
	@Autowired
	private TracerLog tracer;
	@Autowired
	private HousingFundYiWuService housingFundYiWuService;
	...
	public ResultData<TaskHousing> housingFundLogin(@RequestBody MessageLoginForHousing messageLoginForHousing) throws Exception {
		...
		save(taskHousing);
//		
		housingFundYiWuService.login(messageLoginForHousing, taskHousing);
	...
	public ResultData<TaskHousing> housingFundCrawler(@RequestBody MessageLoginForHousing messageLoginForHousing) {
		...
		save(taskHousing);
		housingFundYiWuService.crawler(messageLoginForHousing, taskHousing);
		result.setData(taskHousing);
		return result;
	}
