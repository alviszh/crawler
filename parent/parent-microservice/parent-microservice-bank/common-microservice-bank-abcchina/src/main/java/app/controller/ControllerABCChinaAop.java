...
public class ControllerABCChinaAop {
	...
	public TaskBank loginAgent(@RequestBody BankJsonBean bankJsonBean) throws Exception {
		TaskBank taskBank;
		tracerLog.qryKeyValue("农业银行（信用卡）集群的调用...", bankJsonBean.getTaskid());
		try {
			taskBank = agentService.postAgent(bankJsonBean, "/bank/abcchina/credit/login", 180000L);
	...
	public TaskBank sendSmsAgent(@RequestBody BankJsonBean bankJsonBean)
		...
		tracerLog.qryKeyValue("农业银行（信用卡）发送验证码集群的调用...", bankJsonBean.getTaskid());
		TaskBank taskBank;
		try {
			taskBank = agentService.postAgent(bankJsonBean, "/bank/abcchina/credit/sendSms");
		} catch (RuntimeException e) {
			taskBank = taskBankStatusService.changeStatus(BankStatusCode.BANK_AGENT_ERROR.getPhase(),
					BankStatusCode.BANK_AGENT_ERROR.getPhasestatus(), BankStatusCode.BANK_AGENT_ERROR.getDescription(),
					BankStatusCode.BANK_AGENT_ERROR.getError_code(), true, bankJsonBean.getTaskid());
	...
	public TaskBank sendSms(@RequestBody BankJsonBean bankJsonBean) {
		tracerLog.qryKeyValue("农业银行（信用卡）发送验证码的业务调用...", bankJsonBean.getTaskid());
		try {
			serviceABCChinaAop.sendSms(bankJsonBean);
		} catch (Exception e) {
			tracerLog.addTag("农业银行（信用卡）发送验证码的业务调用异常...", e.getMessage());
		}
		return new TaskBank();
	}
	...
	public TaskBank verifySmsAgent(@RequestBody BankJsonBean bankJsonBean)
			throws IllegalAccessException, NativeException, Exception {
		tracerLog.qryKeyValue("农业银行（信用卡）验证验证码集群的调用...", bankJsonBean.getTaskid());
		TaskBank taskBank;
		try {
			taskBank = agentService.postAgent(bankJsonBean, "/bank/abcchina/credit/verifySms");
		} catch (RuntimeException e) {
			taskBank = taskBankStatusService.changeStatus(BankStatusCode.BANK_AGENT_ERROR.getPhase(),
					BankStatusCode.BANK_AGENT_ERROR.getPhasestatus(), BankStatusCode.BANK_AGENT_ERROR.getDescription(),
					BankStatusCode.BANK_AGENT_ERROR.getError_code(), true, bankJsonBean.getTaskid());
	...
	public TaskBank verifySms(@RequestBody BankJsonBean bankJsonBean) {
		...
		try {
			...
			tracerLog.addTag("农业银行（信用卡）验证验证码的业务调用异常...", e.getMessage());
		}
		return new TaskBank();
	}
	...
	public TaskBank getAllDataAgent(@RequestBody BankJsonBean bankJsonBean)
			throws IllegalAccessException, NativeException, Exception {
		tracerLog.qryKeyValue("农业银行（信用卡）爬取和解析集群的调用...", bankJsonBean.getTaskid());
		TaskBank taskBank;
		try {
			taskBank = agentService.postAgent(bankJsonBean, "/bank/abcchina/credit/getAllData");
		} catch (RuntimeException e) {
			taskBank = taskBankStatusService.changeStatus(BankStatusCode.BANK_AGENT_ERROR.getPhase(),
					BankStatusCode.BANK_AGENT_ERROR.getPhasestatus(), BankStatusCode.BANK_AGENT_ERROR.getDescription(),
					BankStatusCode.BANK_AGENT_ERROR.getError_code(), true, bankJsonBean.getTaskid());
	...
	public TaskBank getAllData(@RequestBody BankJsonBean bankJsonBean) {
		tracerLog.qryKeyValue("农业银行（信用卡）爬取和解析的业务调用...", bankJsonBean.getTaskid());
		try {
			serviceABCChinaAop.getAllData(bankJsonBean);
		} catch (Exception e) {
			tracerLog.addTag("农业银行（信用卡）爬取和解析的业务调用异常...", e.getMessage());
		}
		return new TaskBank();
	}
