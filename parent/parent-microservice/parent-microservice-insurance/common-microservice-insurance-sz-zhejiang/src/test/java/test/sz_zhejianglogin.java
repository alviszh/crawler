...
public class sz_zhejianglogin {
	...
	public static void main(String[] args) throws Exception {
		...
		loginname.setText("18857002671");//18857002671
		loginpwd.setText("xuwang888888");
		HtmlPage page2 = log.click();
		
//		HtmlTextInput loginname2 = (HtmlTextInput) page2.getElementById("loginname");//身份证
//		HtmlPasswordInput loginpwd2 = (HtmlPasswordInput) page2.getElementById("loginpwd");//密码
...
//		HtmlImage image = (HtmlImage) page2.getElementById("captcha_img");
...
//		verifycode.setText(inputValue);
		...
		String city = "衢州市";
//		loginname2.setText("18857002671");//18857002671
//		loginpwd2.setText("xuwang888888");//xuwang888888
		//衢州市 18857002671 xuwang888888
		//丽水市 332501198902260222 pisces226
		//台州市 511602199408281123 gyp1127
		//义乌市  15158984773 19950407whq222
		//金华市  330721198807274225 zxx13757960257
	//	Page page3 = log2.click();

		String html3 = page2.getWebResponse().getContentAsString();
		System.out.println(html3);
                   
		if(html3.indexOf("浙江政务服务网（省级）")!=-1){  //第一次选择（社保查询）
			System.out.println("成功");
			if(city.equals("衢州市")){
				String url0 = "https://puser.zjzwfw.gov.cn/sso/usp.do?action=user";
				Page html = (HtmlPage) getHtml(url0, webClient);
				String asString = html.getWebResponse().getContentAsString();
				Document parse = Jsoup.parse(asString);
				Elements elementsByClass = parse.getElementsByClass("lanmu_sub2_1");
				String text = elementsByClass.get(0).text().trim();//帐号名
				String text1 = elementsByClass.get(3).text().trim();//手机号
				String text2 = elementsByClass.get(5).text().trim();//邮箱号
				String text3 = elementsByClass.get(6).text().trim();//安全等级
				String text4 = elementsByClass.get(7).text().trim();//性别
				System.out.println(text+text1+text2+text3+text4);
				/*try {
					//个人信息
				...
				//个人信息
				/*String urll = "http://sbcx.pcyyhj.zjzwfw.gov.cn/proxy/services/BizData/ZmUxLWJlYzktMDgxMmI2MGQ3NWI3?dataset=sbcx_sheng$sbcx_grjbxx";
				getHtml(urll, webClient);
				Page page2 = getHtml(urll, webClient);
				String htmll = page2.getWebResponse().getContentAsString();
				System.out.println(htmll);
				...
				System.out.println(text+text1+text2+text3+text4);*/
				//养老
				/*String urll3 = "http://sbcx.pcyyhj.zjzwfw.gov.cn/proxy/services/BizData/ZmUxLWJlYzktMDgxMmI2MGQ3NWI3?"
						+ "dataset=sbcx_sheng$sbcx_yljf"
						+ "&limit=500"
						+ "&pageNo=1"
						+ "&pageSize=500";
				...
				System.out.println(htmll3);
				JSONObject object9 = JSONObject.fromObject(htmll3);
				JSONArray obArray = object9.getJSONArray("data");
				for (int i = 0; i < obArray.size(); i++) {
					if(obArray.getJSONObject(i).has("aae002")){
						...
						if(obArray.getJSONObject(i).has("aab004")){
							string15 = obArray.getJSONObject(i).getString("aab004");//公司名称
						}
						String string16 = null;
						if(obArray.getJSONObject(i).has("aae143")){
							string16 = obArray.getJSONObject(i).getString("aae143");//缴费类型
						}
						String string17 = obArray.getJSONObject(i).getString("aae111");//缴纳状态
						System.out.println("缴费年月:"+string12+"\r缴费基数:"+string13+"\r个人缴费金额:"+string14+"\r公司名称:"+
								string15+"\r缴费类型:"+string16+"\r缴纳状态:"+string17);
				...
				//医疗
				String urll4 = "http://sbcx.pcyyhj.zjzwfw.gov.cn/proxy/services/BizData/ZmUxLWJlYzktMDgxMmI2MGQ3NWI3?"
						+ "dataset=sbcx_sheng$sbcx_ybjf"
						+ "&limit=500"
						+ "&pageNo=1"
						+ "&pageSize=500";
				Page page8 = getHtml(urll4, webClient);
				String htmll4 = page8.getWebResponse().getContentAsString();
				System.out.println(htmll4);
				JSONObject object8 = JSONObject.fromObject(htmll4);
				JSONArray obArray8 = object8.getJSONArray("data");
				for (int i = 0; i < obArray8.size(); i++) {
					if(obArray8.getJSONObject(i).has("aae002")){
						String string12 = obArray8.getJSONObject(i).getString("aae002");//缴费年月
						...
						String string14 = obArray8.getJSONObject(i).getString("gryj");//个人缴费金额
						String string15 = null;
						if(obArray8.getJSONObject(i).has("aab004")){
							string15 = obArray8.getJSONObject(i).getString("aab004");//公司名称
						}
						String string16 = null;
						if(obArray8.getJSONObject(i).has("aae143")){
							string16 = obArray8.getJSONObject(i).getString("aae143");//缴费类型
						}
						String string17 = obArray8.getJSONObject(i).getString("aae111");//缴纳状态
						System.out.println("医疗：缴费年月:"+string12+"\r缴费基数:"+string13+"\r个人缴费金额:"+string14+"\r公司名称:"+
								string15+"\r缴费类型:"+string16+"\r缴纳状态:"+string17);
					}else{
						System.out.println("无缴纳记录");
					}
				...
				for (int i = 0; i < obArray8.size(); i++) {
					if(obArray8.getJSONObject(i).has("aae002")){
						...
						String string17 = obArray8.getJSONObject(i).getString("aae111");//缴纳状态
						System.out.println("工伤：缴费年月:"+string12+"\r缴费基数:"+string13+"\r个人缴费金额:"+string14+"\r公司名称:"+
								string15+"\r缴费类型:"+string16+"\r缴纳状态:"+string17);
					}else{
						System.out.println("无缴纳记录");
					}
				...
				for (int i = 0; i < obArray8.size(); i++) {
					if(obArray8.getJSONObject(i).has("aae002")){
						...
						String string17 = obArray8.getJSONObject(i).getString("aae111");//缴纳状态
						System.out.println("生育：缴费年月:"+string12+"\r缴费基数:"+string13+"\r个人缴费金额:"+string14+"\r公司名称:"+
								string15+"\r缴费类型:"+string16+"\r缴纳状态:"+string17);
					}else{
						System.out.println("无缴纳记录");
					}
				...
				for (int i = 0; i < obArray8.size(); i++) {
					if(obArray8.getJSONObject(i).has("aae002")){
						...
						String string17 = obArray8.getJSONObject(i).getString("aae111");//缴纳状态
						System.out.println("失业：缴费年月:"+string12+"\r缴费基数:"+string13+"\r个人缴费金额:"+string14+"\r公司名称:"+
								string15+"\r缴费类型:"+string16+"\r缴纳状态:"+string17);
					}else{
						System.out.println("无缴纳记录");
					}
