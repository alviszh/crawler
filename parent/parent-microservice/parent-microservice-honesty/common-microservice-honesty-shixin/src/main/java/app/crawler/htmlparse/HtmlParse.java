...
public class HtmlParse {
	...
	public static ExecutorRootBean executor_parse(String html) {
		Type type = new TypeToken<ExecutorRootBean>() {
		}.getType();

		ExecutorRootBean jsonObject = gs.fromJson(html, type);
		return jsonObject;
	}
	...
	public static WebParamHonesty<Integer> getIDList(String html) {
		...
		if (es.size() > 0) {
			...
			webParam.setErrormessage("无失信记录");
