...
public class CookieTest {
	...
	private static String phonenum = "15911135456"; 
	private static String passwd = "958575";
//	public static String phonenum = "15210072522"; 
...
//		busiqrydeal();
		needVerifyCode(phonenum);
		chkNumberAction(phonenum); 
//		loadSendflag();
//		captchazh();
//		checkUidAvailable();
...
//		ocr();
	...
	private static void loadToken() throws Exception {
		...
		System.out.println("loadToken html ===>"+page.getWebResponse().getContentAsString());
		JSONObject json = new JSONObject(page.getWebResponse().getContentAsString());
		result = (String) json.get("result");
		
		System.out.println("result  ==============>"+result);
	}

	public static void loadSendflag() throws Exception {
...
//		WebClient webClient = WebCrawler.getInstance().getWebClient();
	...
	public static void needVerifyCode(String account) throws Exception{
		
		String url = "https://login.10086.cn/needVerifyCode.htm?accountType=01&account="+account+"&timestamp="+System.currentTimeMillis();
		WebRequest requestSettings = new WebRequest(new URL(url), HttpMethod.GET); 
		requestSettings.setAdditionalHeader("Accept", "application/json, text/javascript, */*; q=0.01");
		requestSettings.setAdditionalHeader("Accept-Encoding", "gzip, deflate, sdch, br");
		requestSettings.setAdditionalHeader("Accept-Language", "zh-CN,zh;q=0.8");
...
//		WebClient webClient = WebCrawler.getInstance().getWebClient();
	...
	public static void chkNumberAction(String phonenum) throws Exception{
...
//		WebClient webClient = WebCrawler.getInstance().getWebClient();
	...
	public static void ajax(String phonenum) throws Exception{
		...
		ScriptResult scriptResult = page1.executeJavaScript("encrypt('"+passwd+"')");
		
		String sss = scriptResult.getJavaScriptResult().toString();
		
		System.out.println("加密的密码： "+sss);
		
		sss=URLEncoder.encode(sss, "utf-8");//字符编码和文档编码一致
		sss = "ojrKJ1%2B4pGcNeJxzplLUDEPm5HEdgKRvH7yVjQn%2BWczrm30En%2Fh4%2BixSOdcMGsuoa3FksxSg%2FVZ2kRGhvfpjq0de8W2ydofiv%2FfcEmDY%2FZcMJ4CRbZcnpmFb5D1vAQxyvwmXizQ0ijCmzK0z2QkV2ZM%2FKT479KmxWx3AzWxa%2BJoYRH5YHBz9S7sAUOAYxTILouORisT0vbXspc5Ga%2FoyCUqW%2FEh6JgFapJ%2BWM1QTvBi5qUSRNJnpD4Cam6jD6JEG2gFweTvYesugalNdTb%2Bk46J%2FEVtVmqAJb2vXesru84h5XkgpOPwy8cTgmi23uZleIW4SgvAnhrMfXrUbsRIwQQ%3D%3D";
		System.out.println("Encode加密的密码： "+sss);
...
//		WebClient webClient = WebCrawler.getInstance().getWebClient();
		...
		requestSettings.getRequestParameters().add(new NameValuePair("password", sss));
