...
public class HousingSNParse {
	//个人信息
		public static HousingSuiNingUserInfo userinfo_parse(String html){
			HousingSuiNingUserInfo userInfo = new HousingSuiNingUserInfo();
		...
		//缴费流水
		public static List<HousingSuiNingPay> paydetails_parse(String html,TaskHousing taskHousing){
			List<HousingSuiNingPay> list = new ArrayList<HousingSuiNingPay>();
			JsonParser parser = new JsonParser();
			JsonObject object = (JsonObject) parser.parse(html); // 创建JsonObject对象
			//System.out.println(object);
			...
			for (JsonElement acc : accountCardList) {
				...
				HousingSuiNingPay pay = new HousingSuiNingPay();
				pay.setYears(years);
				pay.setType(type);
