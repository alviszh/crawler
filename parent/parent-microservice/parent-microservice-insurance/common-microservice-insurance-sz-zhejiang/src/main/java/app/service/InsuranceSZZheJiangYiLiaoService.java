...
import com.microservice.dao.repository.crawler.insurance.sz.zhejiang.InsuranceSZZheJiangYiLiaoInfoRepository;
...
public class InsuranceSZZheJiangYiLiaoService {
	@Autowired
	...
	@Autowired
	private InsuranceSZZheJiangYiLiaoInfoRepository insuranceSZZheJiangYiLiaoInfoRepository;
	/***
	 * 医疗保险缴费流水
	 * @param parameter
	 * @param taskInsurance
	...
	public Future<String> getyiliaoMsg(InsuranceRequestParameters parameter, TaskInsurance taskInsurance,
		...
		try {
			if(city.indexOf("衢州市")!=-1){
				...
				List<InsuranceSZZheJiangYiLiaoinfo> list = insuranceSZZheJiangParser.getyiliao(html6,parameter.getTaskId());
				if(list!=null){
					insuranceSZZheJiangYiLiaoInfoRepository.saveAll(list);
			...
			tracer.addTag("action.zhejiang.getyiliaoMsg", e.toString());
			taskInsurance.setPhase(InsuranceStatusCode.INSURANCE_CRAWLER_YILIAO_MSG_SUCCESS.getPhase());
			taskInsurance.setPhase_status(InsuranceStatusCode.INSURANCE_CRAWLER_YILIAO_MSG_SUCCESS.getPhasestatus());
			taskInsurance.setDescription(InsuranceStatusCode.INSURANCE_CRAWLER_YILIAO_MSG_SUCCESS.getDescription());
