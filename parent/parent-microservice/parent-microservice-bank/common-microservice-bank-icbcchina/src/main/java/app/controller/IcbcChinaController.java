...
@RestController
@Configuration
@RequestMapping("/bank/icbcchina/debitcard") 
public class IcbcChinaController {
	@Autowired
	...
	public TaskBank loginAgent(@RequestBody BankJsonBean bankJsonBean) throws IllegalAccessException, NativeException, Exception {
		...
		try {
			taskBank = agentService.postAgent(bankJsonBean, "/bank/icbcchina/debitcard/login", 300000L); 
		} catch (RuntimeException e) {
			tracer.qryKeyValue("taskid", bankJsonBean.getTaskid());
			tracer.addTag("icbcBankController.debitcard.loginAgent.exception", e.getMessage());
		}
		return taskBank;
	}
	...
	public TaskBank loginIcbcChina(@RequestBody BankJsonBean bankJsonBean){
		...
		try{
			...
			tracer.addTag("crawler.bank.login.exception", e.getMessage());
		}
		return taskBank;
	}
	...
	public TaskBank setSMSCodeAgent(@RequestBody BankJsonBean bankJsonBean) throws IllegalAccessException, NativeException, Exception {
		tracer.addTag("crawler.bank.setSMSCode", bankJsonBean.getTaskid());   
		TaskBank taskBank =  taskBankRepository.findByTaskid(bankJsonBean.getTaskid().trim()); 
		...
		try {
			taskBank =  agentService.postAgentCombo(bankJsonBean, "/bank/icbcchina/debitcard/setSMSCode");
		} catch (RuntimeException e) {
			tracer.qryKeyValue("taskid", bankJsonBean.getTaskid());
			tracer.addTag("icbcBankController.debitcard.setSMSCodeAgent.exception", e.getMessage());
		}
		return taskBank;
	}
	...
	public TaskBank setSMSCode(@RequestBody BankJsonBean bankJsonBean){
		tracer.addTag("crawler.bank.setSMSCode.taskid", bankJsonBean.getTaskid());
