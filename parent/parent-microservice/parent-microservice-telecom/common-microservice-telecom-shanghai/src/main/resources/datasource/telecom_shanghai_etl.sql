CREATE OR REPLACE FUNCTION "public"."telecom_shanghai_etl"("taskid" text)
...
---用户基本信息
		...
		(
			...
			select 
				'telecom_shanghai_userinfo:'|| a.id::TEXT as resource,
				a.taskid as task_id,
				...
				date_part('day',CURRENT_DATE-a.cust_sincedt::TIMESTAMP) as net_in_duration,
...
--缴费信息
		...
		(
			...
			select 
				'telecom_shanghai_payfee:' || a.id::text as resource,
				a.taskid as task_id,
				a.createtime as createtime,
				a.store_in_amount as payfee,
				to_char(a.partner_trans_date::TIMESTAMP,'yyyy-mm-dd') as paytime,
				to_char(a.partner_trans_date::TIMESTAMP,'yyyy-mm') as paymonth,
...
--短信信息
		...
		(
			...
			select 
				'telecom_shanghai_msg:' || a.id::text as resource,
				a.taskid as task_id,
				a.createtime as createtime,
...
--通话记录信息
		...
		(
			...
			select 
				'telecom_shanghai_callresult:'||a.id::text as resource,
				a.taskid as task_id,
				a.createtime as createtime,
				b.phonenum as phone_num,
--				case when a.called_num = b.phonenum then a.calling_num else a.called_num end as his_num,
				...
				case when position('.' in a.called_party_visited_city) <> 0 
							then split_part(a.called_party_visited_city,'.',2) 
...
;;--ALTER FUNCTION "public"."telecom_shanghai_etl"("taskid" text) OWNER TO "TXDB";