CREATE OR REPLACE FUNCTION "public"."telecom_tianjin_etl"("taskid" text)
...
---用户基本信息
		...
		(
			...
			select 
				'telecom_tianjin_userinfo:'|| a.id::TEXT as resource,
				a.taskid as task_id,
				...
				to_char(a.accesstime::timestamp,'yyyy-mm-dd') as net_in_date,
				date_part('day',CURRENT_DATE-a.accesstime::TIMESTAMP) as net_in_duration,				
...
--缴费信息
		...
		(
			...
			select 
				'telecom_tianjin_chargeinfo:' || a.id::text as resource,
				a.taskid as task_id,
				a.createtime as createtime,
				a.chargemoney as payfee,
				to_char(substr(a.chargetime,1,10)::TIMESTAMP,'yyyy-mm-dd') as paytime,
				to_char(substr(a.chargetime,1,10)::TIMESTAMP,'yyyy-mm') as paymonth,
...
--短信信息
		...
		(
			...
			select 
				'telecom_tianjin_smsrecord:' || a.id::text as resource,
				a.taskid as task_id,
				a.createtime as createtime,
...
--套餐（服务）信息
		...
		(
			...
			select
				'telecom_tianjin_business:'|| a.id::TEXT as resource,
				a.taskid as task_id,
				a.createtime as createtime,
...
--通话记录信息
		...
		(
			...
			select 
				'telecom_tianjin_callrecord:'||a.id::text as resource,
				a.taskid as task_id,
				a.createtime as createtime,
				b.phonenum as phone_num,
...
--月账单信息
		...
		(
			...
			select 
				a.taskid as task_id,
				sum(a.expense::NUMERIC) as sum_fee,
				sum(case when b.item_name = '基本套餐费' then a.expense::NUMERIC else 0 end)::text as basic_fee,
				sum(case when b.item_name = '短信彩信费' then a.expense::NUMERIC else 0 end)::text as message_fee,
				sum(case when b.item_name = '国内通话费' then a.expense::NUMERIC else 0 end)::text as normal_call_fee,
				sum(case when b.item_name = '异地通话费' then a.expense::NUMERIC else 0 end)::text as roam_call_fee,
				sum(case when b.item_name = '互联网流量费' then a.expense::NUMERIC else 0 end)::text as flow_fee,
				sum(case when b.item_name = '业务功能费' then a.expense::NUMERIC else 0 end)::text as function_fee,
				sum(case when b.item_name = '其他费用' then a.expense::NUMERIC else 0 end)::text as other_fee,
...
--ALTER FUNCTION "public"."telecom_tianjin_etl"("taskid" text) OWNER TO "TXDB";
