...
function initials() {//公众号排序
    ...
    $.each(initials, function(index, value) {//添加首字母标签
        ...
        SortBox.append('<a href="#" class="list-group-item" style="background-color:#ddd;color:white;font-weight:bold;" id="'+ value +'">' + value + '</a>');
        /*显示右边字母*/
        Initials.find('ul').append('<li>'+ value +'</li>');
    });
    Initials.find('ul').append('<li>#</li>');

    //if(num!=0){SortBox.append('<div class="sort_letter" id="default">#</div>');}
    if(num!=0){SortBox.append('<a href="#" class="list-group-item" style="background-color:#ddd;color:white;font-weight:bold;" id="default">#</div>');}

    for (var i =0;i<SortList.length;i++) {//插入到对应的首字母后面
        var letter=makePy(SortList.eq(i).find(num_name).text().charAt(0))[0].toUpperCase();
        //console.log(SortList.eq(i).find(num_name).text()+"***:"+letter);
        switch(letter){
            case "A":
    ...
    /*点击右边字母*/
    $(".container .initials ul li").click(function(){
        var _this=$(this);
    ...
    $(".initials").find('ul').children('li').each(function(){
