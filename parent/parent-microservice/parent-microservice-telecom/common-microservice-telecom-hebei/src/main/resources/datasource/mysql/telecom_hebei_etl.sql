DROP FUNCTION IF EXISTS telecom_hebei_etl;

CREATE FUNCTION telecom_hebei_etl(mission_id varchar(50)) RETURNS text CHARSET utf8
BEGIN
...
#用户基本信息
		...
		(
			...
			select 
				'telecom_hebei_userinfo:'|| a.id as resource,
				a.taskid as task_id,
				...
				DATEDIFF(now(),replace(replace(replace(j.net_time,'年','-'),'月','-'),'日','')) as net_in_duration,
...
#缴费信息
		...
		(
			...
			select 
				'telecom_hebei_payfee:' || a.id as resource,
				a.taskid as task_id,
				a.createtime as createtime,
...
#短信信息
		...
		(
			...
			select 
				'telecom_hebei_msg:' || a.id as resource,
				a.taskid as task_id,
				a.createtime as createtime,
...
#通话记录信息
		...
		(
			...
			select 
				'telecom_hebei_callresult:'||a.id as resource,
				a.taskid as task_id,
				a.createtime as createtime,
				b.phonenum as phone_num,
