...
import com.microservice.dao.entity.crawler.housing.xining.HousingXiningUserInfo;
...
@Component
public class HousingXiNingParse {
	@Autowired
	...
	public WebParam login(MessageLoginForHousing messageLoginForHousing, TaskHousing taskHousing) {
		...
		tracer.addTag("HousingXiNingParse.login", taskHousing.getTaskid());

		WebClient webClient = WebCrawler.getInstance().getNewWebClient();
		WebParam webParam = new WebParam();
		try {
			String url = "http://218.95.230.61/wt-web/grlogin";
			...
			if (null != searchPage) {
				...
				try {
					...
					tracer.addTag("HousingXiNingParse.login.code.ERROR", taskHousing.getTaskid() + "-----ERROR:" + e);
					e.printStackTrace();
				}
			...
			tracer.addTag("HousingXiNingParse.login:",
	...
	public WebParam getUserInfo(TaskHousing taskHousing) throws Exception {
		...
		tracer.addTag("HousingXiNingParse.getUserInfo", taskHousing.getTaskid());
		...
		try {
			...
			if (null != page) {
				...
				tracer.addTag("HousingXiNingParse.getUserInfo---用户信息" + taskHousing.getTaskid(),
						"<xmp>" + html + "</xmp>");
				HousingXiningUserInfo housingXiningUserInfo = htmlUserInfoParser(taskHousing, html);
				webParam.setPage(page);
				webParam.setHtml(html);
				webParam.setHousingXiningUserInfo(housingXiningUserInfo);
				webParam.setUrl(page.getUrl().toString());
				webParam.setCode(page.getWebResponse().getStatusCode());
				return webParam;
			}
		} catch (Exception e) {
			tracer.addTag("HousingXiNingParse.getUserInfo---ERROR:",
	 ...
	 * @return
	 */
	public HousingXiningUserInfo htmlUserInfoParser(TaskHousing taskHousing, String html) {

		tracer.addTag("HousingXiNingParse.htmlUserInfoParser---info:" + taskHousing.getTaskid(),
				"<xmp>" + html + "</xmp>");
		...
		try {
			...
			if ("true".equals(flag)) {
				...
				HousingXiningUserInfo housingXiningUserInfo = new HousingXiningUserInfo(taskHousing.getTaskid(),
				...
				return housingXiningUserInfo;
			}
		} catch (Exception e) {
			e.printStackTrace();
			tracer.addTag("HousingXiNingParse.htmlUserInfoParser---ERROR:",
					taskHousing.getTaskid() + "---ERROR:" + e.toString());
		}
	 ...
	 * @throws Exception
	 */
	public WebParam<HousingXiningPaydetails> getPay(TaskHousing taskHousing) throws Exception {

		tracer.addTag("HousingXiNingParse.getPay", taskHousing.getTaskid());

		WebParam<HousingXiningPaydetails> webParam = new WebParam<HousingXiningPaydetails>();
		...
		try {
			...
			paramsList.add(new NameValuePair("ndxz", URLEncoder.encode("全部", "UTF-8")));
			...
			paramsList.add(new NameValuePair("totalcount", "10000"));
			paramsList.add(new NameValuePair("pages", "10"));
	
			Page page = getPage(webClient, taskHousing, url, HttpMethod.GET, paramsList, null, null, null);	
			if (null != page) {
				String html = page.getWebResponse().getContentAsString();
				tracer.addTag("HousingXiNingParse.getPay---缴费信息" + taskHousing.getTaskid(), "<xmp>" + html + "</xmp>");

				List<HousingXiningPaydetails> list = htmlPayParser(html, taskHousing);
				webParam.setPage(page);
			...
			tracer.addTag("HousingXiNingParse.getPay---ERROR:", taskHousing.getTaskid() + "---ERROR:" + e.toString());
			e.printStackTrace();
		}
		return null;
	}
	 ...
	 * @return
	 */
	private List<HousingXiningPaydetails> htmlPayParser(String html, TaskHousing taskHousing) {

		
		List<HousingXiningPaydetails> list = new ArrayList<HousingXiningPaydetails>();
		try {
			HousingXiningPaydetails housingXiningPaydetails = null;
			...
			if (obj instanceof JSONObject) {
				...
				housingXiningPaydetails = new HousingXiningPaydetails(taskHousing.getTaskid(), jzrq, gjhtqywlx, fse, fslxe, grzhye, tqyy, tqfs);
				list.add(housingXiningPaydetails);
			...
			e.printStackTrace();
			tracer.addTag("HousingXiNingParse.htmlPayParser---ERROR:",
					taskHousing.getTaskid() + "---ERROR:" + e.toString());
		}
		return list;
	...
	public Page getPage(WebClient webClient, TaskHousing taskHousing, String url, HttpMethod type,
		...
		tracer.addTag("HousingXiNingParse.getPage---url:", url + "---taskId:" + taskHousing.getTaskid());
		...
		Thread.sleep(2500);
		int statusCode = searchPage.getWebResponse().getStatusCode();
		tracer.addTag("HousingXiNingParse.getPage.statusCode:" + statusCode, "---taskid:" + taskHousing.getTaskid());

		if (200 == statusCode) {
			String html = searchPage.getWebResponse().getContentAsString();
			tracer.addTag("HousingXiNingParse.getPage---taskid:",
