...
import app.service.PsbcChinaService;
import app.service.TaskBankStatusService;

@RestController
@Configuration
@RequestMapping("/bank/psbcchina/debitcard")
public class PsbcChinaController extends AgentService {

	@Autowired
	PsbcChinaService psbcChinaService;
	...
	public TaskBank loginAgent(@RequestBody BankJsonBean bankJsonBean) throws Exception {
		...
		try {
			taskBank = agentService.postAgent(bankJsonBean, "/bank/psbcchina/debitcard/login",INTERVAL_TIME);
        } catch (Exception e) {
...
//		TaskBank taskBank = taskBankStatusService.changeStatusLoginDoing(bankJsonBean);
		TaskBank taskBank = psbcChinaService.login(bankJsonBean);
		return taskBank;
	}
...
//				null, false, bankJsonBean.getTaskid());
		...
		taskBank = agentService.postAgentCombo(bankJsonBean, "/bank/psbcchina/debitcard/crawler");
		return taskBank;
	...
	public TaskBank crawler(@RequestBody BankJsonBean bankJsonBean) throws Exception {
		...
		// 无需短信，直接开始爬取（异步）
		TaskBank taskBank = psbcChinaService.getAllData(bankJsonBean); 
	...
	public TaskBank quit(@RequestBody BankJsonBean bankJsonBean){
		TaskBank taskBank = psbcChinaService.quit(bankJsonBean);
		return taskBank;
	}
