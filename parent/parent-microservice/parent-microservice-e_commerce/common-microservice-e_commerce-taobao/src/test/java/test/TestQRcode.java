...
//		driver.get("https://buyertrade.taobao.com/trade/itemlist/list_bought_items.htm");
		...
		/*webClient = getWebClientByWebDriver("member1.taobao.com");
...
//					e1.printStackTrace();
			...
			System.out.println("nick:"+nick+" shopName:"+shopName);
...
//					e.printStackTrace();
	...
	public static void parserTaobaoAddressInfo(String html){
		Document document = Jsoup.parse(html);
		Element tbl_main = document.getElementsByClass("next-table").first();
		Elements trs = tbl_main.select("tr");
		if(trs.size() > 1){
