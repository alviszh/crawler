...
public class HousingBaoTouFutureService  extends HousingBasicService implements ICrawlerLogin,ICrawler{
	public static final Logger log = LoggerFactory.getLogger(HousingBaoTouFutureService.class);
	
	@Autowired
	private LoginAndGetService loginAndGetService;
	@Autowired
	private HousingBaoTouService housingBaoTouService;
	...
	public TaskHousing login(MessageLoginForHousing messageLoginForHousing) {
		...
		String url = "http://www.btgjj.com/login.html";
		Page htmlpage = null;
		try {
...
//			tracer.addTag("e", "baotougangjijin");
		...
		if(htmlpage!=null){
			tracer.addTag("模拟登陆成功html", htmlpage.getWebResponse().getContentAsString());
			if(htmlpage.getWebResponse().getContentAsString().contains("会员登录成功！")){
				tracer.addTag("模拟登陆成功", messageLoginForHousing.getTask_id());
...
//				updateTaskHousing(taskHousing.getTaskid());
				...
				tracer.addTag("登陆失败，请重试", messageLoginForHousing.getTask_id());
...
//			tracer.addTag("e", "baotougangjijin");
	...
	public TaskHousing getAllData(MessageLoginForHousing messageLoginForHousing) {
		...
		try {
			Future<String> future = housingBaoTouService.getResult(messageLoginForHousing, taskHousing, webClient);
		} catch (Exception e) {
