...
import app.service.HousingHanzhongInfoService;
import net.sf.json.JSONArray;
import net.sf.json.JSONObject;
import net.sf.json.util.JSONTokener;

@Component
public class HousingHanzhongParse {
	...
	public WebParam login(MessageLoginForHousing messageLoginForHousing, TaskHousing taskHousing) {
		...
		tracer.addTag("HousingHanzhongParse.login", taskHousing.getTaskid());

		WebClient webClient = WebCrawler.getInstance().getNewWebClient();
		WebParam webParam = new WebParam();
		try {
			String url = "http://zfgjj.hanzhong.gov.cn:8080/wt-web/grlogin";
			...
			if (null != searchPage) {
				...
				try {
					...
					tracer.addTag("HousingHanzhongParse.login.code.ERROR", taskHousing.getTaskid() + "-----ERROR:" + e);
					e.printStackTrace();
				}
			...
			tracer.addTag("HousingHanzhongParse.login:",
	 ...
	 * @throws Exception
	 */
	public WebParam<HousingHanzhongUserInfo> getUserInfo(TaskHousing taskHousing) throws Exception {

		tracer.addTag("HousingHanzhongParse.getUserInfo", taskHousing.getTaskid());

		WebParam<HousingHanzhongUserInfo> webParam = new WebParam<HousingHanzhongUserInfo>();

		try {
			WebClient webClient = addcookie(taskHousing);
			
			String url = "http://zfgjj.hanzhong.gov.cn:8080/wt-web/jcr/jcrkhxxcx_mh.service";
			...
			if (null != page) {
				...
				tracer.addTag("HousingHanzhongParse.getUserInfo---用户信息" + taskHousing.getTaskid(),
						"<xmp>" + html + "</xmp>");
				List<HousingHanzhongUserInfo> list = htmlUserInfoParser(taskHousing, html);
				if(list.size()>0){
					housingHanzhongInfoService.getPay(taskHousing,list.get(0));
				}
			...
			tracer.addTag("HousingHanzhongParse.getUserInfo---ERROR:",
	 ...
	 * @return
	 */
	public List<HousingHanzhongUserInfo> htmlUserInfoParser(TaskHousing taskHousing, String html) {

		tracer.addTag("HousingHanzhongParse.htmlUserInfoParser---info:" + taskHousing.getTaskid(),
				"<xmp>" + html + "</xmp>");
		...
		try {
			...
			e.printStackTrace();
			tracer.addTag("HousingHanzhongParse.htmlUserInfoParser---ERROR:",
					taskHousing.getTaskid() + "---ERROR:" + e.toString());
		}
	 ...
	 * @throws Exception
	 */
	public WebParam<HousingHanzhongPay> getPay(TaskHousing taskHousing,HousingHanzhongUserInfo housingHanzhongUserInfo) throws Exception {

		tracer.addTag("HousingHanzhongParse.getPay", taskHousing.getTaskid());

		WebParam<HousingHanzhongPay> webParam = new WebParam<HousingHanzhongPay>();

		try {
			WebClient webClient = addcookie(taskHousing);

			String url = "http://zfgjj.hanzhong.gov.cn:8080/wt-web/jcr/jcrxxcxzhmxcx.service";
			...
			paramsList.add(new NameValuePair("grxx", housingHanzhongUserInfo.getGrzh()));
			paramsList.add(new NameValuePair("pageNum", "1"));
			paramsList.add(new NameValuePair("page", "1"));
			paramsList.add(new NameValuePair("startPage", "0"));
			paramsList.add(new NameValuePair("pageSize", "100000"));
			paramsList.add(new NameValuePair("size", "20"));
			...
			if (null != page) {
				String html = page.getWebResponse().getContentAsString();
				tracer.addTag("HousingHanzhongParse.getPay---缴费信息" + taskHousing.getTaskid(), "<xmp>" + html + "</xmp>");

				List<HousingHanzhongPay> list = htmlPayParser(html, taskHousing);
				webParam.setPage(page);
			...
			tracer.addTag("HousingHanzhongParse.getPay---ERROR:", taskHousing.getTaskid() + "---ERROR:" + e.toString());
			e.printStackTrace();
		}
		return null;
	}
	 ...
	 * @return
	 */
	private List<HousingHanzhongPay> htmlPayParser(String html, TaskHousing taskHousing) {

		
		List<HousingHanzhongPay> list = new ArrayList<HousingHanzhongPay>();
		try {
			HousingHanzhongPay housingHanzhongPay = null;
			...
			if (obj instanceof JSONObject) {
				...
				housingHanzhongPay = new HousingHanzhongPay(taskHousing.getTaskid(), jzrq, gjhtqywlx, fse, fslxe,tqyy, tqfs);
				list.add(housingHanzhongPay);
			...
			e.printStackTrace();
			tracer.addTag("HousingHanzhongParse.htmlPayParser---ERROR:",
					taskHousing.getTaskid() + "---ERROR:" + e.toString());
		}
		return null;
	...
	public Page getPage(WebClient webClient, TaskHousing taskHousing, String url, HttpMethod type,
		...
		tracer.addTag("HousingHanzhongParse.getPage---url:", url + "---taskId:" + taskHousing.getTaskid());
		...
		int statusCode = searchPage.getWebResponse().getStatusCode();
		tracer.addTag("HousingHanzhongParse.getPage.statusCode:" + statusCode, "---taskid:" + taskHousing.getTaskid());

		if (200 == statusCode) {
			String html = searchPage.getWebResponse().getContentAsString();
			tracer.addTag("HousingHanzhongParse.getPage---taskid:",
