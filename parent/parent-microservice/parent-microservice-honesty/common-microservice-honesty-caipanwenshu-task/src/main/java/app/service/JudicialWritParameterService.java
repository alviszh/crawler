...
public class JudicialWritParameterService {
	...
	public WebParam getParameter(HttpProxyBean httpProxyBean,HonestyJsonBean honestyJsonBean, int i) throws Exception{
		if(i<6){
			...
			if(contentAsString!=null){
				...
				if(guid!=null){
					...
					judicialWritTaskRepository.getguid(guid,"guid获取成功",honestyJsonBean.getTaskid());
				}else{
					judicialWritTaskRepository.getguid(null, "guid获取失败", honestyJsonBean.getTaskid());
					return null;
				}
				...
				if(cookies.size()>0){
					...
					if(vl5x!=null){
						...
						judicialWritTaskRepository.getVl5x(vl5x,"vl5x获取成功！",honestyJsonBean.getTaskid());
					}else{
						judicialWritTaskRepository.getVl5x(null,"vl5x获取失败！",honestyJsonBean.getTaskid());
						return null;
					}
				}else{
					judicialWritTaskRepository.getVl5x(null,"vl5x获取失败！",honestyJsonBean.getTaskid());
				...
				if(number!=null){
					...
					judicialWritTaskRepository.getNumber(number,"参数获取完成！",honestyJsonBean.getTaskid());
				}else{
					judicialWritTaskRepository.getNumber(null,"number获取失败！",honestyJsonBean.getTaskid());
					return null;
				}
			}else{
				System.out.println("获取参数重试第"+i+"次");
				Thread.sleep(1000);
				getParameter(httpProxyBean, honestyJsonBean, i+1);
			}
