...
@RestController
@RequestMapping("/insurance/luoyang")
public class InsuranceLuoYangController {
	...
	public TaskInsurance login(@RequestBody InsuranceRequestParameters parameter) {
		...
		tracer.addTag("InsuranceluoyangController.login:开始登录", parameter.toString());
		// 通过taskid查出对应表中的数据
	...
	public TaskInsurance crawler(@RequestBody InsuranceRequestParameters parameter) {
		...
		tracer.addTag("InsuranceluoyangController.crawler:检测Task", parameter.toString());
		tracer.addTag("InsuranceluoyangController.crawler:开始爬取", parameter.toString());
		// 执行业务
