			...
			if (asXml.indexOf("登录失败") != -1) {
				...
				WebRequest webRequestwdzl22 = new WebRequest(new URL(wdzlurl22), HttpMethod.POST);
				  List<NameValuePair> params22 = new ArrayList<NameValuePair>();
				   ...
				   params22.add(new NameValuePair("page", "/2017/details.jsp"));
				   params22.add(new NameValuePair("httpSessionId", ""));
				   params22.add(new NameValuePair("scriptSessionId", "AA0CBE9FB90164F9E0E55CF74FCC9338823"));
				   ...
				   params22.add(new NameValuePair("c0-param0", "string:DETAILS_SERVICE"));   //参数2
				   params22.add(new NameValuePair("c0-param1", "boolean:false"));
				   params22.add(new NameValuePair("c0-e1", "string:SEND_SMS_CODE"));
				
				   params22.add(new NameValuePair("c0-param2", "Object_Object:{method:reference:c0-e1}"));
				   params22.add(new NameValuePair("batchId", "4"));
...
//					webRequestwdzl2.setAdditionalHeader("Content-Length", "1166"); 
				...
				String contentAsString = wdzl22.getWebResponse().getContentAsString();
				System.out.println("请求发送验证码获取页面响应是："+contentAsString);
