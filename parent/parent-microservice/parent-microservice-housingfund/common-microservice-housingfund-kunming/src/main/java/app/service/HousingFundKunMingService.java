...
public class HousingFundKunMingService extends HousingBasicService implements ICrawlerLogin{
	@Value("${loginhost}") 
	...
	@Autowired
	private HousingFundKunMingCrawlerService housingFundKunMingCrawlerService;
	@Autowired
	...
	public TaskHousing getAllData(MessageLoginForHousing messageLoginForHousing) {
		...
		housingFundKunMingCrawlerService.getUserInfo(taskHousing);
		return taskHousing;
	}
	@Override
	...
	public TaskHousing login(MessageLoginForHousing messageLoginForHousing) {
		TaskHousing taskHousing =findTaskHousing(messageLoginForHousing.getTask_id());
		try {
			String url="http://"+loginHost+"/kmnbp/index.jsp?flg=1";
			...
			if(null!=hPage){
				//获取图片验证码
				url="http://"+loginHost+"/kmnbp/vericode.jsp";
				...
				if(page!=null){
					...
					//验证登录信息
					url="http://"+loginHost+"/kmnbp/per.login";
					...
					if(null!=page){
						...
						HousingKunMingHtml loginHtml = new HousingKunMingHtml();
						...
						housingKunMingHtmlRepository.save(loginHtml);			
