...
public class LoginAndGetService {
	 ...
	 * @throws Exception
	 */
	public WebParamTelecomByChrome<TelecomStarlevel> getStarlevel(TaskMobile taskMobile,int k) throws Exception {
		...
		try {
			...
			try {
				...
				System.out.println("json==========="+("<xmp>" + json + "</xmp>").replaceAll("<xmp>", "").replaceAll("</xmp>", ""));
				JSONObject jsonObj = JSONObject.fromObject(("<xmp>" + json + "</xmp>").replaceAll("<xmp>", "").replaceAll("</xmp>", ""));
			...
			try {
				...
				System.out.println("json=="+("<xmp>" + json + "</xmp>").replaceAll("<xmp>", "").replaceAll("</xmp>", ""));
				JSONObject jsonObj2 = JSONObject.fromObject(("<xmp>" + json + "</xmp>").replaceAll("<xmp>", "").replaceAll("</xmp>", ""));
			...
			if (null != page) {
				...
				tracerLog.addTag("TelecomShanxiParser.getStarlevel---用户星级服务信息" + taskMobile.getTaskid(),
						"<xmp>" + html + "</xmp>");
				WebParamTelecomByChrome<TelecomStarlevel> webParam  = TelecomParseCommon.starlevel_Parser(html, taskMobile);
