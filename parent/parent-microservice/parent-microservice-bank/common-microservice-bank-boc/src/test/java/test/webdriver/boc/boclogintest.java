...
public class boclogintest {
	...
	public static String loginChrome() throws Exception {
		...
		//Thread.sleep(10000L);// 等待10秒，根据当前URL是否还是登录页面，判断登录是否成功，等待5秒也是等待登录成功后的页面信息加载
		...
		Page page = getTranFlow(url, webClient, countid);
		BocParse2.transFlow_parse(page.getWebResponse().getContentAsString());

		/*Page page = getUserInfo(url, webClient);
	...
	public static String loginChrome2() throws Exception {
		WebDriver driver = intiChrome();
		...
		driver.findElement(By.id("txt_username_79443")).sendKeys("230623199303110444");
		ele1.click();
		...
		Thread.sleep(5000L);// 等待10秒，根据当前URL是否还是登录页面，判断登录是否成功，等待5秒也是等待登录成功后的页面信息加载
		String htmlsource3 = driver.getPageSource();
		System.out.println("3333333333333333333333333333" + htmlsource3);
		wait2 = new FluentWait<WebDriver>(driver).withTimeout(60, TimeUnit.SECONDS).pollingEvery(2, TimeUnit.SECONDS)
				.ignoring(NoSuchElementException.class);
		ele2 = wait2.until(new Function<WebDriver, WebElement>() {
			public WebElement apply(WebDriver driver) {
		...
		if (currentPageURL.indexOf("https://ebsnew.boc.cn/boc15/welcome_ele.html") != -1) {
			...
			htmlsource3 = driver.getPageSource();
			System.out.println(htmlsource3);
			Document doc = Jsoup.parse(htmlsource3);
			String error_texg = doc.select("span#msgContent").text();
			System.out.println("=============error_texg=============" + error_texg);
	...
	public static Page getTranFlow(String url, WebClient webClient, String countid) throws Exception {
		WebRequest webRequest = new WebRequest(new URL(url), HttpMethod.POST);
		webClient.setJavaScriptTimeout(50000);
		webClient.getOptions().setTimeout(50000); // 15->60
		// webClient.addRequestHeader("Accept", "*/*");
		...
		webRequest.setRequestBody(
				"{\"header\":{\"local\":\"zh_CN\",\"agent\":\"WEB15\",\"bfw-ctrl\":\"json\",\"version\":\"\",\"device\":\"\",\"platform\":\"\",\"plugins\":\"\",\"page\":\"\",\"ext\":\"\"},"
						...
						+ "\"" + countid.trim() + "\","
						+ "\"params\":{\"accountSeq\":\"36861666\",\"currency\":\"001\",\"cashRemit\":\"\","
						+ "\"startDate\":\"2017/05/24\"," + "\"endDate\":\"" + "2017/10/31\"" + "," + "\"" + "pageSize"
						+ "\":\"10\",\"_refresh\":\"true\",\"currentIndex\":\"0\"}}]}");
		Page searchPage = webClient.getPage(webRequest);
		System.out.println(searchPage.getWebResponse().getContentAsString());
		return searchPage;
	}
	...
	public static Page getUserInfoOpendate(String url, WebClient webClient) throws Exception {
		WebRequest webRequest = new WebRequest(new URL(url), HttpMethod.POST);
		webClient.setJavaScriptTimeout(50000);
		webClient.getOptions().setTimeout(50000); // 15->60
		// webClient.addRequestHeader("Accept", "*/*");
		...
		webRequest.setRequestBody(
				"{\"header\":{\"local\":\"zh_CN\",\"agent\":\"WEB15\",\"bfw-ctrl\":\"json\",\"version\":\"\",\"device\":\"\",\"platform\":\"\",\"plugins\":\"\",\"page\":\"\",\"ext\":\"\"},\"request\":[{\"id\":8,\"method\":\"PsnAccBocnetQryDebitDetail\",\"conversationId\":null,\"params\":{\"accountSeq\":\"36861666\"}}]}");
		Page searchPage = webClient.getPage(webRequest);
		System.out.println(searchPage.getWebResponse().getContentAsString());
		return searchPage;
	}

	public static Page getUserInfoSingleLimit(String url, WebClient webClient) throws Exception {
		WebRequest webRequest = new WebRequest(new URL(url), HttpMethod.POST);
		webClient.setJavaScriptTimeout(50000);
		webClient.getOptions().setTimeout(50000); // 15->60
		// webClient.addRequestHeader("Accept", "*/*");
		...
		webRequest.setRequestBody(
				"{\"header\":{\"local\":\"zh_CN\",\"agent\":\"WEB15\",\"bfw-ctrl\":\"json\",\"version\":\"\",\"device\":\"\",\"platform\":\"\",\"plugins\":\"\",\"page\":\"\",\"ext\":\"\"},\"request\":[{\"id\":10,\"method\":\"PsnAccBocnetQryEcashDetail\",\"conversationId\":null,\"params\":{\"accountSeq\":\"36861666\"}}]}");
		Page searchPage = webClient.getPage(webRequest);
		System.out.println(searchPage.getWebResponse().getContentAsString());
		return searchPage;
	}
