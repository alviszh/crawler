...
public class HousingZYParse {
	//个人信息
	public static HousingZiYangUserInfo userinfo_parse(String html){
		HousingZiYangUserInfo userInfo = new HousingZiYangUserInfo();
	...
	//缴费流水
	public static List<HousingZiYangPay> paydetails_parse(String html,TaskHousing taskHousing){
		List<HousingZiYangPay> list = new ArrayList<HousingZiYangPay>();
		JsonParser parser = new JsonParser();
		JsonObject object = (JsonObject) parser.parse(html); // 创建JsonObject对象
		//System.out.println(object);
		...
		for (JsonElement acc : accountCardList) {
			...
			HousingZiYangPay pay = new HousingZiYangPay();
			pay.setYears(years);
			pay.setType(type);
