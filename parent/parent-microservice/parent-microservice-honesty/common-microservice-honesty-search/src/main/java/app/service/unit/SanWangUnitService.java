...
public class SanWangUnitService {
	...
	/**
	 * 
	 * 项目名称：common-microservice-honesty-search 所属包名：app.search.unit 类描述：
	 * 获取详情页数据，不归为解析类 创建人：hyx 创建时间：2018年10月18日
	 * 
	 * @version 1 返回值 Future<NewsContent>
	 */
	@Async("ExecutorForSearch") // 配置类中的方法名
	public Future<NewsContent> getContent(NewsListJson newsListJson, SearchTask searchTask)
		...
		if (doc == null) {
			...
			tracerLog.System(newsListJson.getLinkUrl() + "  " + "详情页获取 耗时        当前共计程序耗时s：",
					(endTime_get - startTime_get) / 1000 + "s");

			String urlbasic = new String(toUtf8String(doc.baseUri()).getBytes("gbk"), "UTF8");

			newsListJson.setLinkUrl(urlbasic);

			newsContent.setContent(doc.text());
			try {
				newsContent.setSensitivekey(searchUnitService.matchSensitive(doc.text()).toString()
