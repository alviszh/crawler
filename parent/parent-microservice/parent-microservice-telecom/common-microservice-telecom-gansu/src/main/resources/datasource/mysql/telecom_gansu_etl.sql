DROP FUNCTION IF EXISTS telecom_gansu_etl;

CREATE FUNCTION telecom_gansu_etl(mission_id varchar(50)) RETURNS text CHARSET utf8
BEGIN
...
#用户基本信息
		...
		(
			...
			select 
				'telecom_gansu_userinfo:'|| a.id as resource,
				a.taskid as task_id,
...
#缴费信息
		...
		(
			...
			select 
				'telecom_gansu_pay:' || a.id as resource,
				a.taskid as task_id,
				a.createtime as createtime,
				a.money as payfee,
...
#短信信息
		...
		(
			...
			select 
				'telecom_gansu_message:' || a.id as resource,
				a.taskid as task_id,
				a.createtime as createtime,
...
#套餐（服务）信息
		...
		(
			...
			select
				'telecom_gansu_business:'|| a.id as resource,
				a.taskid as task_id,
				a.createtime as createtime,
...
#通话记录信息
		...
		(
			...
			select 
				'telecom_gansu_call:'||a.id as resource,
				a.taskid as task_id,
				a.createtime as createtime,
