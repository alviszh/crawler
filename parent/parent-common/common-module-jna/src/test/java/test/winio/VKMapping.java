...
public class VKMapping {
	...
	public static int toScanCodeUppercase(String key){
		System.out.println("toScanCodeUppercase------>"+key);
		try { 
			int vkey = User32.MapVirtualKeyA(getMapValue(getMapUpValue(key)).intValue()); 
			System.out.println("vkey--------"+vkey); 
			return vkey;
		} catch (Exception e) {
			System.out.println(key+"-------toScanCodeUppercase Exception--------"+e.toString());
			return 0;
		}
	}
