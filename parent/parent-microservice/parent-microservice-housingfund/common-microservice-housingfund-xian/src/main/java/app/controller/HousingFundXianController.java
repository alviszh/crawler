...
import app.service.HousingFundXianService;



@RestController  
@Configuration
@RequestMapping("/housing/xian")
public class HousingFundXianController extends HousingBasicController{
	
	@Autowired
	private TracerLog tracer;
	@Autowired
	private   HousingFundXianService housingFundXianService;
	...
	public ResultData<TaskHousing> housingFundLogin(@RequestBody MessageLoginForHousing messageLoginForHousing) throws Exception {
		...
		tracer.addTag("action.housingFund.xian.login.start",messageLoginForHousing.getTask_id());
		...
		housingFundXianService.login(messageLoginForHousing, taskHousing);
	...
	public ResultData<TaskHousing> housingFundCrawler(@RequestBody MessageLoginForHousing messageLoginForHousing) {
		...
		save(taskHousing);
		housingFundXianService.crawler(messageLoginForHousing, taskHousing);
		result.setData(taskHousing);
		return result;
	}
