...
@RestController
@Configuration
@RequestMapping("/bank/cmbchina/creditcard")
public class CmbChinaController extends AgentService {
	...
	public TaskBank loginAgent(@RequestBody BankJsonBean bankJsonBean) throws Exception {
		...
		try {
			taskBank = agentService.postAgent(bankJsonBean, "/bank/cmbchina/creditcard/login");
        } catch (Exception e) {
...
//				null, false, bankJsonBean.getTaskid());
		...
		taskBank = agentService.postAgentCombo(bankJsonBean, "/bank/cmbchina/creditcard/crawler");
		return taskBank;
	...
	public TaskBank crawler(@RequestBody BankJsonBean bankJsonBean) throws Exception {
		...
		// 无需短信，直接开始爬取（异步）
		TaskBank taskBank = cmbChinaService.getAllData(bankJsonBean); // 这个方法会抛出未捕获的异常
																		// 待处理
																		// todo
		return taskBank;
	}
...
//				BankStatusCode.BANK_SEND_CODE_DONING.getDescription(), null, false, bankJsonBean.getTaskid());
		...
		// 发送短信 (同步)
		taskBank = agentService.postAgentCombo(bankJsonBean, "/bank/cmbchina/creditcard/sendSmsCode");
		return taskBank;
	}
...
//				BankStatusCode.BANK_VALIDATE_CODE_DONING.getDescription(), null, false, bankJsonBean.getTaskid());
		...
		// 验证短信(同步)
		taskBank = agentService.postAgentCombo(bankJsonBean, "/bank/cmbchina/creditcard/smsverfiy");
		return taskBank;
	}
