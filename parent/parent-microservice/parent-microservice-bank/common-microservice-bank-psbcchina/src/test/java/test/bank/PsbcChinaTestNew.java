...
//			kahao();
	...
	public static void kahao() {
		String json = "{\"ec\":\"0\",\"em\":\"\",\"iAccountInfoList\":[{\"accountNo\":\"6221882600107620508\",\"functionBitmap\":\"00000000011100000000000000000000\",\"shortAccountNo\":\"6221882600107620508\",\"liveAccount\":\"0\",\"signFlag\":\"1\",\"channelFlag\":\"0\",\"accountOpenNode\":\"11009163\",\"accountType\":\"03\",\"openOrg\":\"23011010\",\"defaultFlag\":\"11\",\"cardFlag\":null,\"accountAlias\":\"\",\"accountFlag\":null,\"bkBookFlg\":\"2\",\"PROD_LEVEL\":null,\"hostSubFlag\":null,\"seqNo\":\"0\"}],\"sequenceNo\":\"32889721\",\"windowId\":\"null\"}";
	...
	public static void zhanghu() {
		File file = new File("C:\\Users\\Administrator\\Desktop\\yue.txt");
		String json = txt2String(file);
		...
		System.out.println("账户类型----" + account_type);
		// 开户机构
		...
		try {
			...
			if (elementsByTag.size() > 5) {
				...
				System.out.println("开户日期----" + account_opening);
				// 账户状态
	...
	public static void login() throws Exception {
		...
		String accountNum = "12345678910";

		String password = "123456789";
		// Thread.sleep(1000L);
		String loginType = "PHONE_NUM";
		if (loginType.equals(StatusCodeLogin.IDNUM)) {
//			driver.findElement(By.id("cerType")).click();
		} else if (loginType.equals(StatusCodeLogin.PHONE_NUM)) {
			driver.findElement(By.id("mobType")).click();
		} else if (loginType.equals(StatusCodeLogin.ACCOUNT_NUM)) {
			driver.findElement(By.id("logType")).click();
		} else if (loginType.equals(StatusCodeLogin.CARD_NUM)) {
			driver.findElement(By.id("accType")).click();
		}
		driver.findElement(By.id("logonId")).sendKeys(accountNum);
		InputTab();
		Thread.sleep(1000L);
		VirtualKeyBoard.KeyPressEx(password,1000);// 输入密码
		Thread.sleep(1000L);
		
		// 验证码
		// String path = WebDriverUnit.saveImg(driver,
		// By.xpath("//*[@id=\"_tokenImg\"]/../../.."));
		String path = WebDriverUnit.saveImg(driver, By.xpath("//*[@id=\"verifyImg\"]/../.."));
//		String path = WebDriverUnit.saveImg(driver, By.xpath("//*[@id='loginContent']/div/div/div[1]/ul/li[3]"));
...
//				System.out.println(cookie.getName() + "-------cookies--------" + cookie.getValue());
			...
			driver.findElement(By.xpath("//*[@id=\"acc\"]/div[1]/div[1]/div/span[2]")).click();
			Thread.sleep(1000L);
...
//			 System.out.println(html);
			...
			JavascriptExecutor driver_js = ((JavascriptExecutor) driver);
			Object executeScript = driver_js.executeScript("return $.lily.CONFIG_SESSION_ID;");
			...
			if (main != null) {
				...
				String tipLogon = driver.findElement(By.id("tipLogonId")).getText();
				if (StringUtils.isNoneBlank(tipLogon)) {
					System.out.println("tipLogon---------------" + tipLogon);
				} else {
					System.out.println("tipLogon---------------无错误信息");
				}

				String tipPassword = driver.findElement(By.id("tipPasswordId")).getText();
				if (StringUtils.isNoneBlank(tipPassword)) {
					System.out.println("tipPasswordId---------------" + tipPassword);
				} else {
					System.out.println("tipPasswordId---------------无错误信息");
				}
				String tipCheck = driver.findElement(By.id("tipCheckCode")).getText();
				if (StringUtils.isNoneBlank(tipCheck)) {
					System.out.println("tipCheckCode---------------" + tipCheck);
				} else {
					System.out.println("tipCheckCode---------------无错误信息");
				}
			}
...
//				driver.quit();
				...
				System.out.println("errorfinfo---------------未知情况");
			}
		}
