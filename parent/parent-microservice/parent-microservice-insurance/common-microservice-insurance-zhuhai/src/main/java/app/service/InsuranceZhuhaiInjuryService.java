...
public class InsuranceZhuhaiInjuryService {
	...
	public void getInjury(WebClient webClient, String taskid) {
		...
		for(int i = 1; i<30; i++){
			String maternityUrl = "https://www.zhldj.gov.cn/zhrsClient/social.do?doMethod=listFee&insured_kind=4&curr_page="+String.valueOf(i);
			try {
				HtmlPage html = (HtmlPage) insuranceService.getHtml(maternityUrl,webClient);
				tracer.addTag("工伤信息页面 "+i, html.getWebResponse().getContentAsString());
				List<InsuranceZhuhaiUnify> list = insuranceZhuhaiParser.parserInsurance(html,"工伤保险",taskid);
				if(null != list){
					insuranceZhuhaiUnifyRepository.saveAll(list);
					insuranceService.changeCrawlerStatus("【工伤信息】采集成功", InsuranceStatusCode.INSURANCE_CRAWLER_GONGSHANG_MSG_SUCCESS.getPhase(), 
							200, taskInsurance);
				}else{
					insuranceService.changeCrawlerStatus("【工伤信息】采集成功", InsuranceStatusCode.INSURANCE_CRAWLER_GONGSHANG_MSG_SUCCESS.getPhase(), 
							200, taskInsurance);
				...
				tracer.addTag("工伤医疗信息页面失败！", e.getMessage());
				insuranceService.changeCrawlerStatus("【工伤信息】获取超时", InsuranceStatusCode.INSURANCE_CRAWLER_GONGSHANG_MSG_SUCCESS.getPhase(), 
						404, taskInsurance);
				continue;
			}		
		}
