DROP FUNCTION IF EXISTS telecom_shanxi3_etl;

CREATE FUNCTION telecom_shanxi3_etl(mission_id varchar(50)) RETURNS text CHARSET utf8
BEGIN
...
#用户基本信息
		...
		(
			...
			select 
				'telecom_shanxi3_userinfo:'|| a.id as resource,
				a.taskid as task_id,
...
#缴费信息
		...
		(
			...
			select 
				'telecom_shanxi3_chargeinfo:' || a.id as resource,
				a.taskid as task_id,
				a.createtime as createtime,
				a.chargemoney as payfee,
				date_format(a.chargetime,'%y-%m-%d') as paytime,
				date_format(a.chargetime,'%y-%m') as paymonth,
...
#短信信息
		...
		(
			...
			select 
				'telecom_shanxi3_smsrecord:' || a.id as resource,
				a.taskid as task_id,
				a.createtime as createtime,
...
#通话记录信息
		...
		(
			...
			select 
				'telecom_shanxi3_callrecord:'||a.id as resource,
				a.taskid as task_id,
				a.createtime as createtime,
				b.phonenum as phone_num,
...
#月账单信息
		...
		(
			...
			select 
				a.taskid as task_id,
				sum(a.totalcost) as sum_fee,
				sum(case when b.item_name = '基本套餐费' then a.totalcost else 0 end) as basic_fee,
				sum(case when b.item_name = '短信彩信费' then a.totalcost else 0 end) as message_fee,
				sum(case when b.item_name = '国内通话费' then a.totalcost else 0 end) as normal_call_fee,
				sum(case when b.item_name = '异地通话费' then a.totalcost else 0 end) as roam_call_fee,
				sum(case when b.item_name = '互联网流量费' then a.totalcost else 0 end) as flow_fee,
				sum(case when b.item_name = '业务功能费' then a.totalcost else 0 end) as function_fee,
				sum(case when b.item_name = '其他费用' then a.totalcost else 0 end) as other_fee,
