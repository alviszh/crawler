...
import app.service.ServiceCGB_credit_ChinaAop;
import app.service.TaskBankStatusService;

@RestController
@Configuration
@RequestMapping("/bank/cgbchina/credit")
public class ControllerCGB_credit_ChinaAop {
	@Autowired
	private TracerLog tracerLog;
	@Autowired
	private TaskBankStatusService taskBankStatusService;
	@Autowired
	private ServiceCGB_credit_ChinaAop serviceCGB_credit_ChinaAop;
	...
	public TaskBank loginAgent(@RequestBody BankJsonBean bankJsonBean) throws  Exception { 
		tracerLog.qryKeyValue("广发银行（信用卡）集群的调用...", bankJsonBean.getTaskid());   
		TaskBank taskBank;
		try {
			taskBank = agentService.postAgent(bankJsonBean, "/bank/cgbchina/credit/login",180000L); 
	...
	public TaskBank debitcardLoginCcbChina(@RequestBody BankJsonBean bankJsonBean) {
		...
		tracerLog.qryKeyValue("广发银行（信用卡）业务登录的调用...", bankJsonBean.getTaskid());

		TaskBank taskBank = taskBankStatusService.changeStatusLoginDoing(bankJsonBean);
		try {
			serviceCGB_credit_ChinaAop.login(bankJsonBean);
		} catch (Exception e) {
			tracerLog.addTag("广发银行（信用卡）业务登录的调用异常", e.getMessage());
		}
		return taskBank;
	}
	...
	public TaskBank getAllDataAgent(@RequestBody BankJsonBean bankJsonBean){ 
		tracerLog.qryKeyValue("广发银行（信用卡）爬取和解析集群的调用...", bankJsonBean.getTaskid());
		TaskBank taskBank;
		try {
			taskBank = agentService.postAgent(bankJsonBean, "/bank/cgbchina/credit/getAllData");
		} catch (RuntimeException e) {
			taskBank = taskBankStatusService.changeStatus(BankStatusCode.BANK_AGENT_ERROR.getPhase(),
					BankStatusCode.BANK_AGENT_ERROR.getPhasestatus(), BankStatusCode.BANK_AGENT_ERROR.getDescription(),
					BankStatusCode.BANK_AGENT_ERROR.getError_code(), true, bankJsonBean.getTaskid());
	...
	public TaskBank getAllData(@RequestBody BankJsonBean bankJsonBean){
		tracerLog.qryKeyValue("广发银行（信用卡）爬取和解析的业务调用...", bankJsonBean.getTaskid());
		try {
			serviceCGB_credit_ChinaAop.getAllData(bankJsonBean);
		} catch (Exception e) {
			tracerLog.addTag("广发银行（信用卡）爬取和解析的业务调用异常...", e.getMessage());
		}
		return new TaskBank();
	}
	...
	public TaskBank quit(@RequestBody BankJsonBean bankJsonBean) {
		TaskBank taskBank = serviceCGB_credit_ChinaAop.quit(bankJsonBean);
		return taskBank;
	}
}
