...
public class HousingJiAnFutureService extends HousingBasicService implements ICrawlerLogin,ICrawler{
	public static final Logger log = LoggerFactory.getLogger(HousingJiAnFutureService.class);
	
	@Autowired
	private LoginAndGetService loginAndGetService;
	@Autowired
	private HousingJiAnService housingJiAnService;
	@Async
	@Override
	public TaskHousing login(MessageLoginForHousing messageLoginForHousing) {
		...
		Page htmlpage = null;
		try {
			htmlpage = loginAndGetService.loginByIDNUM(webClient, messageLoginForHousing.getUsername().trim(), messageLoginForHousing.getNum().trim(), messageLoginForHousing.getPassword().trim(), messageLoginForHousing.getCountNumber());
		} catch (Exception e) {
...
//			tracer.addTag("e", "baotougangjijin");
			tracer.addTag("登录异常", e.getMessage());
			save(taskHousing);
			return taskHousing;
		}
		if(htmlpage!=null){
			tracer.addTag("登陆html", htmlpage.getWebResponse().getContentAsString());
			if(htmlpage.getWebResponse().getContentAsString().contains("余额")){
...
//			tracer.addTag("e", "baotougangjijin");
	...
	public TaskHousing getAllData(MessageLoginForHousing messageLoginForHousing) {
		...
		try {
			Future<String> future = housingJiAnService.getResult(messageLoginForHousing, taskHousing, webClient);
		} catch (Exception e) {
