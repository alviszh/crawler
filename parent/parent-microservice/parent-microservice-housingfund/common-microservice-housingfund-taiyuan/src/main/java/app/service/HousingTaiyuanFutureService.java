...
public class HousingTaiyuanFutureService extends HousingBasicService  implements ICrawlerLogin{

	@Autowired
	private HousingTaiyuanParse housingTaiyuanParse;

	@Autowired
	private HousingTaiyuanInfoService housingTaiyuanInfoService;
	...
	public TaskHousing loginTwo(MessageLoginForHousing messageLoginForHousing, TaskHousing taskHousing, Integer count)
		...
		tracer.addTag("HousingTaiyuanFutureService.login", messageLoginForHousing.getTask_id());
		JSONObject jsonObject = JSONObject.fromObject(messageLoginForHousing);
		try {
			if (null != taskHousing) {
				WebParam webParam = housingTaiyuanParse.login(messageLoginForHousing, taskHousing);

				if (null == webParam) {
					tracer.addTag("HousingTaiyuanFutureService.login", messageLoginForHousing.getTask_id() + "登录页获取超时！");
					...
					tracer.addTag("HousingTaiyuanFutureService.login--次数:" + count,
							messageLoginForHousing.getTask_id() + html);
					if (!html.contains("太原市公积金个人网厅登录页")) {
						...
						if (username_tip.contains("用户名无效")) {
							...
							tracer.addTag("HousingTaiyuanFutureService.login--失败次数:" + count,
	...
	public TaskHousing getAllData(MessageLoginForHousing messageLoginForHousing) {
		...
		housingTaiyuanInfoService.getUserInfo(taskHousing);
		housingTaiyuanInfoService.getMsg(taskHousing);
		return null;
	}
	...
	public TaskHousing login(MessageLoginForHousing messageLoginForHousing) {
		...
		try {
			...
			tracer.addTag("HousingTaiyuanFutureService.login:" , messageLoginForHousing.getTask_id()+"---ERROR:"+e.toString());
			e.printStackTrace();
		}
		return null;
	}
